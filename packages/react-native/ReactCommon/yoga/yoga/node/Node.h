 ...
 public:
    ...
    public:
        ...
        struct Iterator {
            ...
            Iterator(const Node* node, size_t childIndex) : node_(node), childIndex_(childIndex) {}
            Iterator(const Node* node, size_t childIndex, Backtrack&& backtrack) : node_(node), childIndex_(childIndex), backtrack_(std::move(backtrack)) {}
        ...
        private:
            void next() {
                if (childIndex_ + 1 >= node_->getChildCount()) {
                    // if the current node has no more children, try to backtrack and visit its successor
                    if (backtrack_.empty()) {
                        // if there are no nodes to backtrack to, the last node has been visited
                        node_ = nullptr;
                        childIndex_ = -1;
                    } else {
            ...
            void skipContentsNodes() {
                ...
                while (currentNode->style().display() == Display::Contents && currentNode->getChildCount() > 0) {
                    // if it has display: contents set, it shouldn't be returned but its children should in its place
                    // push the current node and child index so that the current state can be restored when backtracking
                    backtrack_.push_back({node_, childIndex_});
                    // traverse the child
                    node_ = currentNode;
                    childIndex_ = 0;

                    // repeat until a node without display: contents is found in the subtree or a leaf is reached
